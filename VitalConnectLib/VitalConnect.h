...
extern VitalConnectObserverType kVCIOberserverAutoscanStatusChange;
...
/// Specifies that the observer would like to receive Sleep Stage notifications.
extern VitalConnectObserverType kVCIObserverSleepStage;
...
typedef enum {
    ...
    /// Run this application with no server connectivity required.
    kVitalConnectServerNone,
} VitalConnectServerEnvironment;
...
} VitalConnectNotificationStatus;
...
} VitalConnectThresholdWatchSeverity;
...
} VitalConnectThresholdWatchBound;
...
@optional
- (void) WatchManagerAddedWatch:(NSObject<VitalConnectWatch>*)watch;
/// Called by library upon removal of a watch.
@optional
- (void) WatchManagerRemovedWatch:(NSObject<VitalConnectWatch>*)watch;
/// Called by library upon triggering of a watch.
@optional
- (void) WatchManagerTriggeredWatch:(NSObject<VitalConnectWatch>*)watch byNotification:(NSString*)localNotificationId At:(NSDate*)triggerTime;
/// Called by library upon updating of a watch.
@optional
- (void) WatchManagerChangedWatch:(NSObject<VitalConnectWatch>*)watch;
@end
...
typedef enum
{
	/// The Sleep Engine is ready for collection
	kVCISleepStateReady,
	/// The Sleep Engine is collecting sleep data
	kVCISleepStateCollecting,
	/// Sleep Engine collection has been paused
	kVCISleepStateCollectingPaused,
	/// The Sleep Engine has collected requisite data
	kVCISleepStateCollected,
	/// The Sleep Engine is processing sleep data
	kVCISleepStateProcessing,
	/// Sleep Engine processing has been halted.
	kVCISleepStateProcessingHalted,
} VCSleepState;
...
typedef enum
{
	/// No Error
	kVCISleepAllOk,
	/// No result produced because of insufficient data
	kVCISleepInsuffData,
} VCSleepStatusEnum;

@interface VitalConnectSleepStatus : NSObject
{
@public
	VCSleepState sleepState;
	VCSleepStatusEnum	sleepStatusEnum;
	NSString *message;
	NSDate *timeStamp;
}
@end

@protocol VitalConnectSleepListener <NSObject>
-(void) sleepStatusChanged:(VitalConnectSleepStatus*)status;
-(void) hypnogramCompleted:(NSDate*)timeStamp;
@end
...
} VitalConnectFirmwareUpgradeError;
...
@public
    ...
    /// Approximate percentage complete to flash current image. Includes download time and time to flash.
 ...
 * @since V1.7
 */
+(VitalConnectManager*) createVitalConnect:(NSString*) product_key environment: (VitalConnectServerEnvironment) environment rootFileDir:(NSString*) rootDir encrypted:(BOOL) encrypted;
/**
 ...
 */
-(void)enableHighFrequencyData:(BOOL)bEnable;

/**
 * Get raw data server storage flag
 */
-(BOOL)highFrequencyDataEnabled;
 ...
 * @since 1.95
 */
- (void)scanAndConnectSensorForName:(NSString*)deviceName forSensorSource:(NSString*)guid pageConnect:(BOOL) pageConnect;
...
/**
 * Converts a sample number to a date from the currently active sensor.  The results of this function are undefined when
 * not connected to a sensor.
 * @retval The date corresponding to the supplied sample.
 * @param sampleCount The sample to convert.
 */
-(NSDate*)getDateForSample:(unsigned int)sampleCount;
 ...
 * @since V1.9
 */
- (NSObject<VitalConnectWatch>*) getWatchForSensorSource: (NSString *)guid withLocalId:(NSString*)localId;
 ...
 * @since 1.7
 */
- (void) deleteWatch:(NSObject<VitalConnectWatch>*)watch;
...
#pragma mark - Sleep Analysis
- (void) addSleepListener:(id<VitalConnectSleepListener>) sleepListener;
- (void) delSleepListener:(id<VitalConnectSleepListener>) sleepListener;
- (void) startSleepCapture;
- (void) stopSleepCapture;
- (void) startSleepProcessing;
- (void) haltSleepProcessing;
- (void) resumeSleepProcessing;
- (VCSleepState) getSleepState;
- (void) resetSleepLibrary;
...
/**
 * Has the relay library synced the high frequency state for the sensor?
 */
@property (readonly) BOOL isHighFrequencyStateSynced;

// Ask a connected module to blink its LED for a short period of time. @since V1.7
@property BOOL blinkLED;

/// A connected module's high frequency data status: ECG and and acceleromter (MEMS)
@property BOOL highFrequencyData;
...
/// The current Module Type for the sensor. 
@property (readonly, strong, nonatomic) NSString *moduleType;
